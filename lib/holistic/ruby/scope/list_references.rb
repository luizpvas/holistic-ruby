...
module Holistic::Ruby::Scope
  module ListReferences
    ...
    def call(application:, cursor:)
      ...
      query_references_recursively = ->(scope) do
        references = application.references.list_references_to(scope.fully_qualified_name)

        references_to_child_scopes = scope.children.flat_map { query_references_recursively.call(_1) }

        references + references_to_child_scopes
      end

      references = query_references_recursively.call(scope)
